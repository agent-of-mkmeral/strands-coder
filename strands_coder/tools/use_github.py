...
def get_github_token(use_pat_token: bool = False) -> str | None:
    """Get GitHub token from environment variables.

    Args:
        use_pat_token: If True, use PAT_TOKEN instead of GITHUB_TOKEN (default: False)

    Returns:
        GitHub token string or None if not found
    """
    if use_pat_token:
        return os.environ.get("PAT_TOKEN", "")
...
def use_github(
    ...
    Environment Variables:
        - GITHUB_TOKEN: GitHub token used by default (prevents workflow loops)
        - PAT_TOKEN: Personal Access Token used when use_pat_token=True
        - BYPASS_TOOL_CONSENT: Set to "true" to skip confirmation prompts

    Token Selection:
        - use_pat_token=False (default): Uses GITHUB_TOKEN - safe for workflows ✅
        - use_pat_token=True: Uses PAT_TOKEN - may trigger recursive workflows ⚠️

    Note: GITHUB_TOKEN prevents recursive workflow triggers in GitHub Actions.
          Only set use_pat_token=True when you need PAT-level permissions.
    """
    ...
    # Get GitHub token
    github_token = get_github_token(use_pat_token)
    if not github_token:
        return {
            "status": "error",
            "content": [
