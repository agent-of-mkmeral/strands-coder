...
from tools.github_tools import list_issues, add_issue_comment, create_issue

agent = Agent(tools=[list_issues, add_issue_comment, create_issue])

# List open issues in repository
result = agent.tool.list_issues(state="open", repo="owner/repo")

# Add comment to an issue
result = agent.tool.add_issue_comment(
    issue_number=42,
...
def log_inputs(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        console.print(
            Panel(
                ", ".join(params), title=f"[bold blue]{func_name}", border_style="blue"
            )
        )
...
def create_issue(title: str, body: str = "", repo: str | None = None) -> str:
    ...
    if isinstance(result, str):
        console.print(
            Panel(escape(result), title="[bold red]Error", border_style="red")
        )
        return result

    message = f"Issue created: #{result['number']} - {result['html_url']}"
    console.print(
        Panel(escape(message), title="[bold green]Success", border_style="green")
    )
    return message
...
def list_issues(state: str = "open", repo: str | None = None) -> str:
    ...
    if not issues:
        message = (
            f"No {state} issues found in {repo or os.environ.get('GITHUB_REPOSITORY')}"
        )
        console.print(
            Panel(escape(message), title="[bold yellow]Info", border_style="yellow")
        )
        return message
...
def get_issue_comments(
    issue_number: int, repo: str | None = None, since: str | None = None
) -> str:
    ...
    result = _github_request(
        "GET", f"issues/{issue_number}/comments", repo, params=params
    )
    if isinstance(result, str):
        console.print(
            Panel(escape(result), title="[bold red]Error", border_style="red")
    ...
    if not result:
        message = f"No comments found for issue #{issue_number}" + (
            f" updated after {since}" if since else ""
        )
        console.print(
            Panel(escape(message), title="[bold yellow]Info", border_style="yellow")
        )
    ...
    console.print(
        Panel(
            escape(output),
            title=f"[bold green]💬 Issue #{issue_number} Comments",
            border_style="blue",
        )
    )
    return output
...
def add_issue_comment(
    issue_number: int, comment_text: str, repo: str | None = None
) -> str:
    ...
    """
    result = _github_request(
        "POST", f"issues/{issue_number}/comments", repo, {"body": comment_text}
    )
    if isinstance(result, str):
        console.print(
            Panel(escape(result), title="[bold red]Error", border_style="red")
...
def create_pull_request(
    title: str, head: str, base: str, body: str = "", repo: str | None = None
) -> str:
...
def list_pull_requests(state: str = "open", repo: str | None = None) -> str:
    ...
    output = (
        f"Pull Requests ({state}) in {repo or os.environ.get('GITHUB_REPOSITORY')}:\n"
    )
    for pr in result:
...
@tool
@log_inputs
def get_pr_review_and_comments(
    pr_number: int,
    ...
    try:
        ...
        if since:
            ...
            for thread in pr_data["reviewThreads"]["nodes"]:
                has_newer_comment = any(
                    datetime.fromisoformat(c["updatedAt"].replace("Z", "+00:00"))
                    > cutoff
            ...
            pr_data["comments"]["nodes"] = [
                ...
                if datetime.fromisoformat(c["updatedAt"].replace("Z", "+00:00"))
                > cutoff
            ]
        ...
        for review_id, review_info in review_threads.items():
            ...
            for thread in review_info["threads"]:
                first_comment = thread["comments"]["nodes"][0]
                line_info = (
                    f":{first_comment['line']}"
                    if first_comment.get("line")
                    else " (Comment on file)"
                )
                status = "✅ RESOLVED" if thread["isResolved"] else "🔄 OPEN"

                output += (
                    f"   📍 Thread ({status}): {first_comment['path']}{line_info}\n"
                )
                ...
                if first_comment.get("diffHunk") and first_comment.get("line"):
                    ...
                    output += (
                        f"      Code context (lines {start_line}-{target_line}):\n"
                    )
                    ...
                    for diff_line in diff_lines:
                        if diff_line.startswith("@@"):
                            ...
                            if start_line <= current_new_line <= target_line:
                                output += (
                                    f"       +{current_new_line}: {diff_line[1:]}\n"
                                )
                        elif diff_line.startswith("-"):
                            pass
                        elif diff_line.startswith(" "):
                ...
                for root_comment in root_comments:
                    comment_header = (
                        f"      💬 {root_comment['author']['login']} "
                        f"(updated: {root_comment['updatedAt']}) "
                        f"[Comment ID: {root_comment['fullDatabaseId']}]:\n"
                    )
                    output += comment_header
                    ...
                    replies = [
                        ...
                        for c in comments
                        if c.get("replyTo")
                        and c["replyTo"].get("id") == root_comment["id"]
                    ]
                    if replies:
        ...
        console.print(
            Panel(
                escape(output),
                title=f"[bold green]PR #{pr_number} Review Data",
                border_style="blue",
            )
        )
        return output
...
def reply_to_review_comment(
    pr_number: int, comment_id: int, reply_text: str, repo: str | None = None
) -> str:
    ...
    console.print(
        Panel(
            escape(reply_details),
            title="[bold green]✅ Reply Added",
            border_style="green",
        )
    )
    return message
...
@log_inputs
def add_sub_issue(
    parent_issue_number: int, sub_issue_number: int, repo: str | None = None
) -> str:
    """Adds a sub-issue relationship by commenting on the parent issue.

    Args:
        parent_issue_number: The parent issue number
        sub_issue_number: The sub-issue number to link
        repo: GitHub repository in the format "owner/repo" (optional; falls back to env var)

    Returns:
        Result of the operation
    """
    comment_text = f"Sub-issue: #{sub_issue_number}"
    result: str = add_issue_comment(parent_issue_number, comment_text, repo)

    if "Error:" in result:
        return result

    message = (
        f"Added sub-issue #{sub_issue_number} to parent-issue #{parent_issue_number}"
    )
    console.print(
        Panel(escape(message), title="[bold green]Success", border_style="green")
    )
